...
class ObjectPattern extends SimplePattern {
    ...
    static _get_predicate_function(obj) {
        return src => Object.entries(obj).every(([prop_name, predicator]) => src.hasOwnProperty(prop_name) && P.create(predicator).predicate(src[prop_name]));
    }
}
...
const m = Matcher.create(['123', x => console.log(x), ['@whatever', '@anything'], x => console.log(x), { code: '@code' }, x => console.log(x), P.as(x => x === 100, 'goodname'), x => console.log(x), '@final', x => console.log(x)]);

m.match(100);
m.match('123');
m.match([333, 999]);

const matcher = Matcher.create([[x => x > 100, 'xixi'], 'Literal match [>100, xixi]', P.string, 'Your input is a string', x => x === 100, x => x + 200, 23333, 'exactly 23333', P.greaterThanTen, 'This is a number and is greater than ten', [P.string, P.greaterThanTen], 'Complicated array!', P.nonEmptyArr, 'nonempty array', {
    status: 200,
    content: P.string.as('ct')
}, src => '200 and have content! status code' + src.ct, Cases.AcceptAllAlwaysThrowCase]);
